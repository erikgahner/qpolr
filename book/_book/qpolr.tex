...
% Everything below added by CII
  ...
  \texttt{R} is by far the best software you can use if you want to
  conduct quantitative analyses of political data. Importantly, data
  analysis is no longer restricted to analyzing survey data but also
  social media data, texts, event data, images, geographic data, and so
  forth. For that and other reasons listed below, \texttt{R} is a great
  tool to learn.
  ...
  output. In doing this, we will not only teach you how to get existing
  data into \texttt{R}, but also how to collect your own data.
  ...
  a bad way: if you are used to, say, SPSS, things are not as easy as they
  used to be. Then in a good way: once you learn how to do different tasks
  in \texttt{R}, you will be ashamed when you look back at the old you
  doing analyses in SPSS or something just as embarrasing.
  
  In this chapter you will find an introduction to the basics \texttt{R}.
  The introduction takes place in three steps. First, we ask the obvious
  and important question, \emph{why} \texttt{R}? Second, we help you
  install what you need. Third, we introduce the logic of doing thuings in
  \texttt{R} so you are ready for the chapters to come. Have fun!
  ...
  of \texttt{R}. You can compare \texttt{R} to an iPhone. If you didn't
  have the possibility to install apps on the iPhone, its functionality
  would be limited. In \texttt{R}, just as with iPhones, you have several
  apps (in \texttt{R} called packages), you can install and use in order
  to make life easier.
  
  Third, some of the most beautiful figures you will find today when you
  open the newspaper are created in \texttt{R}. Big media outlets such as
  The New York Times, BBC and FiveThirtyEight use \texttt{R} to create
  figures. Specifically, they use the package \texttt{ggplot2}, a very
  popular package used to create figures. We will work with this package
  later.
  ...
  \texttt{R} is a popular software and in great demand meaning that you
  will not be the first (nor the last) to experience specific issues in
  \texttt{R}. Accordingly, you will find a lot of help on Google and other
  places to a much greater extent than for other types of software.
  ...
  commands in a script (more about this later). So, while you do not see a
  pedagogical graphical user interface in \texttt{R} with a limited set of
  buttons to click, this is more of an advantage than a limitation. Again,
  at first sight, this might seem confusing and frustrating, but we
  promise that, once you get used to it, you are a better scientist.
  
  \section{\texorpdfstring{Installing
  \texttt{R}}{Installing R}}\label{installing-r}
  
  We will actually install two programmes. First \texttt{R} and then
  RStudio. You can compare \texttt{R} to the engine in a car. We call this
  the \texttt{R} language. You can compare RStudio to the beautiful car in
  which you have the engine. We call this the graphical user interface.
  
  To install \texttt{R}, follow this procedure:
  \begin{enumerate}
  ...
  However, you do not need to open \texttt{R} or anything yet. The only
  thing we will open is RStudio, which we will install next.
  ...
  is also available for free. Here, we want to install RStudio Desktop
  with an Open Source License. To install RStudio, follow these steps:
  \begin{enumerate}
  \def\labelenumi{\arabic{enumi}.}
  \tightlist
  \item
  ...
  RStudio, you will see a graphical interface as in Figure
  \ref{fig:interface}.
  \begin{figure}
  ...
  you four windows as you can see in Figure \ref{fig:interfaceexplain}.
  \begin{figure}
  ...
  As you can see, the background is dark in the RStudio environment in
  Figure \ref{fig:interfaceexplain}. The first time you open RStudio, the
  background will be white. If you want to change the colours of your
  RStudio environment, go to \texttt{Tools} \(\rightarrow\)
  \texttt{Global\ Options...} \(\rightarrow\) \texttt{Appearance}.
  ...
  can mark the code and press \texttt{CTRL+R} (Windows) or
  \texttt{CMD+ENTER} (Mac). Then it will run the part of the script you
  have marked. Insert the code below in your script, mark it, run it and
  see how the output shows up in the console:
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  document everything you do in your script. Do also remember to add space
  between your lines of code. This will make it easier for you to read as
  your script gets longer.
  ...
  We mentioned that one of the key advantages of using \texttt{R} is the
  package system. In \texttt{R}, a package is a collection of data and
  functions that makes it easier for you to do what you want. Againm,
  while \texttt{R} is similar to an iPhone, all the packages are
  comparable to apps. The sky is the limit and the only thing you need to
  learn now is how to install and load packages.
  ...
  throughout this book. More specifically, we will work within the
  tidyverse (Hadley Wickham, 2017). You can read more about tidyverse at
  \href{http://tidyverse.org/}{tidyverse.org}. To intall this package
  type:
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  install a package. If you forget this, you will get an error. Make sure
  that you type it exactly as noted above. If you forget a letter
  ...
  same time if you don't need them. For this reason, most scripts
  (i.e.~your window where you have all your code) begin with loading the
  packages that you need. To load a package, we use the function
  \texttt{library()}:
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  \ref{fig:interfacescript}. In other words, do not include the
  \texttt{library("")} line in the middle of your script, but have them
  all at the same time in the beginning of your script.
  \begin{figure}
  ...
  script. It is always good to save your script so you can get back to it
  a later stage.
  
  \section{Errors and help}\label{errors-and-help}
  
  As noted above, you will encounter problems and issues when you work in
  \texttt{R}. Sadly, there are many potential reasons to why your script
  might not be working. Your version of \texttt{R} or/and RStudio might be
  too old or too new, you might be using a function that has a mistake,
  you might be having a small typo, you might not have the data in the
  right format etc.
  ...
  problems with a specific package, also the name of the package. For
  example, if you have a problem with creating a bar chart with the
  \texttt{ggplot2} package, a search string in Google could be
  \texttt{ggplot2\ bar\ chart} and not just \texttt{R\ bar\ chart}.
  ...
  window: write \texttt{2+2} and run the code (mark the code and press
  \texttt{CTRL+R} on Windows or \texttt{CMD+ENTER} on Mac). This should
  ...
  \texttt{R} user (go put that on your CV before you continue). In other
  words, knowing how to use \texttt{R} is not a binary category where you
  either can use \texttt{R} or not, but a continuum where you will always
  be able to learn more. That's great news! However, that also means that
  you will always be able to learn more.
  
  \section{Numbers as data}\label{numbers-as-data}
  
  Next, we will have to learn about variable assignments and in particular
  ...
  To save something in an object (e.g.~a variable), we need to use the
  \emph{assignment operator}, \texttt{\textless{}-}, which basically tells
  \texttt{R} that anything on the right side of the operator should be
  assigned to the object on the left side. Let us try to save the number 2
  in the object \texttt{x}.
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{x <-}\StringTok{ }\DecValTok{2}
  \end{Highlighting}
  \end{Shaded}
  Now \texttt{x} will return the number 2 whenever we write \texttt{x}.
  Let us try to use our object in different simple operations. Write the
  ...
  \NormalTok{x }\OperatorTok{*}\StringTok{ }\DecValTok{2}   \CommentTok{# x times 2}
  
  \NormalTok{x }\OperatorTok{*}\StringTok{ }\NormalTok{x   }\CommentTok{# x times x}
  
  \NormalTok{x }\OperatorTok{+}\StringTok{ }\NormalTok{x   }\CommentTok{# x plus x}
  \end{Highlighting}
  \end{Shaded}
  If it is working, \texttt{R} should return the values \texttt{2},
  \texttt{4}, \texttt{4} and \texttt{4}. If you change the object
  \texttt{x} to have the number 3 instead of 2
  (\texttt{x\ \textless{}-\ 3}) and run the script again, you should get
  different results.\footnote{More specifically, \texttt{3}, \texttt{6},
    \texttt{9} and \texttt{6}.} This is great as you only need to change a
  single number to change the output from the whole procedure.
  Accordingly, when you are working with scripts, try to save as much you
  can in objects, so you only need to change information once, if you want
  to make changes. This will reduce the likelihood of making mistakes.
  
  We can also use our object to create other objects. In the example
  below, we will create a new object \texttt{y}. This object returns the
  sum of \texttt{x} and 7.
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  right away. To get the output, we can type \texttt{y}.
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{y}
  \end{Highlighting}
  ...
  parenthesis as we do below. This tells \texttt{R} that we do not only
  want to save some information in the object \texttt{y}, but that we also
  want to see what is saved in \texttt{y}.
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  \emph{concatenate} or \emph{combine}. Remember that everything happening
  in \texttt{R} happens with functions. A vector can look like this:
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  the same type.\footnote{\texttt{c()} creates a vector with \emph{all}
    elements in the parenthesis. Since a vector can only have one type of
    data, and not both numbers and text (cf.~the next section),
    \texttt{c()} will ensure that all values are reduced to the level all
    values can work with. Consequently, if just one value is a letter and
    not a number, all values in the vector will be considered text.} We
  can save any vector in an object. In the code below we save four numbers
  (14, 6, 23, 2) in the object \texttt{x}.
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# save 14, 6, 23 and 2 in the object x}
  ...
  \CommentTok{# show the output of x}
  \NormalTok{x}
  \end{Highlighting}
  \end{Shaded}
  \begin{verbatim}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# calculate x times 2}
  ...
  just after the object (no space between the name of the object and the
  brackets!). By placing the number 3 in the brackets we get the third
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{party <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Conservative Party"}\NormalTok{, }\StringTok{"Labour Party"}\NormalTok{, }\StringTok{"Scottish National Party"}\NormalTok{, }
         \StringTok{"Liberal Democrats"}\NormalTok{, }\StringTok{"Democratic Unionist Party"}\NormalTok{, }\StringTok{"Sinn Féin"}\NormalTok{) }
  ...
  function \texttt{View()} (do note the capital letter V - not v). Again,
  \texttt{R} is very (case) sensitive.
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  \end{Shaded}
  \begin{figure}
  \includegraphics[width=0.8\linewidth]{fig/View} \caption{Data frame with View(), RStudio}\label{fig:View}
  \end{figure}
  ...
  \end{Highlighting}
  \end{Shaded}
  If you cannot remember the destination, you can use the menu to find the
  folder you want to have as your working directory as shown in Figure
  \ref{fig:setwd}.
  \begin{figure}
  \includegraphics[width=0.5\linewidth]{fig/setwd} \caption{How to change the working directory}\label{fig:setwd}
  \end{figure}
  ...
  not have it, you can use the script below to create the data frame
  again.
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{party <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Conservative Party"}\NormalTok{, }\StringTok{"Labour Party"}\NormalTok{, }\StringTok{"Scottish National Party"}\NormalTok{, }
  ...
  6                 Sinn Féin     Gerry Adams   0.7     7            3
  \end{verbatim}
  Alternatively, you can use the \texttt{desc()} function.
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{arrange}\NormalTok{(uk2017, }\KeywordTok{desc}\NormalTok{(votes))}
  \end{Highlighting}
  \end{Shaded}
  \begin{verbatim}
                        party          leader votes seats seats_change
  ...
  \texttt{essurvey} package (Cimentada, 2018). If you do not have a free
  user, the first step is to go online and create a user:
  ...
  Next, install and load the package \texttt{manifestoR} (Lewandowski \&
  Merz, 2018).
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  use the \texttt{vdem} package (Coppedge et al., 2017). This package is
  not on \texttt{CRAN}, and accordingly, we cannot use
  \texttt{install.packages()} to install it. Instead, we will have to use
  the function \texttt{install\_github()} as it is on GitHub. In order to
  do this, you need to have the package \texttt{devtools}. To install this
  package, you can uncomment the first line below. The second line says
  that we are using the \texttt{install\_github()} function from the
  \texttt{devtools} package (with \texttt{::}).
  \begin{Shaded}
  \begin{Highlighting}[]
  ...
  \end{Highlighting}
  \end{Shaded}
  \section{\texorpdfstring{Data: World Development Indicators
  (\texttt{WDI})}{Data: World Development Indicators (WDI)}}\label{data-world-development-indicators-wdi}
  
  To use data from the World Bank's World Development Indicators, we can
  use the \texttt{WDI} package (Arel-Bundock, 2018). For more information
  on the World Development Indicators, see
  \url{https://datacatalog.worldbank.org/dataset/world-development-indicators}.
  First, install and load the package.
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# install the package}
  \KeywordTok{install.packages}\NormalTok{(}\StringTok{"WDI"}\NormalTok{)}
  
  \CommentTok{# load the package}
  \KeywordTok{library}\NormalTok{(}\StringTok{"WDI"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  To search for data in the WDI, you can use the \texttt{WDIsearch()}
  function. In the example below, we search for data on GDP.
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{WDIsearch}\NormalTok{(}\StringTok{"gdp"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  This returns the indicator and name of the data in WDI. We can see that
  the indicator for GDP per capita, PPP (constant 2005 international \$)
  is \texttt{NY.GDP.PCAP.PP.KD}. To save the data, use the function
  \texttt{WDI()}, where you specify the indicator as well as the countries
  and years you want the data from.
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{wdi <-}\StringTok{ }\KeywordTok{WDI}\NormalTok{(}\DataTypeTok{indicator=}\StringTok{"NY.GDP.PCAP.PP.KD"}\NormalTok{, }
             \DataTypeTok{country=}\KeywordTok{c}\NormalTok{(}\StringTok{"US"}\NormalTok{, }\StringTok{"GB"}\NormalTok{), }
             \DataTypeTok{start =} \DecValTok{1960}\NormalTok{, }
             \DataTypeTok{end =} \DecValTok{2012}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  \section{Data: GitHub repositories}\label{data-github-repositories}
  
  A lot of data today is available in GitHub repositories. To get data
  from GitHub, one good package to use is the package \texttt{RCurl}. The
  first thing to do is to load the package (and if you have not already
  installed the package, you need to do this prior to loading it).
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{library}\NormalTok{(}\StringTok{"RCurl"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  Next, find the dataset on GitHub that you would like to use. When you
  find the dataset, you should click on `Raw' to get to the raw dataset,
  as shown in Figure \ref{fig:githubdata}.
  \begin{figure}
  \includegraphics[width=0.9\linewidth]{fig/githubdata} \caption{How to get to the raw dataset file on GitHub}\label{fig:githubdata}
  \end{figure}
  Copy the url of the raw dataset and use the \texttt{getURL()} function
  in \texttt{R} to load the content of the dataset into \texttt{R}. In the
  example below, we load data on Danish opinion polls and save it in the
  object \texttt{gh\_url}.
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{gh_url <-}\StringTok{ }\KeywordTok{getURL}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/erikgahner/polls/master/polls.csv"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  Last, to get a data frame with the data, and as the dataset is in a
  \texttt{.csv} format, we use \texttt{read.csv()} to save the dataset in
  the object \texttt{gh\_data}.
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{gh_data <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\DataTypeTok{text =}\NormalTok{ gh_url)}
  \end{Highlighting}
  \end{Shaded}
  ...
  election in the United Kingdom. The table is depicted in Figure
  \ref{fig:wikieptable}.
  \begin{figure}
  \includegraphics[width=0.7\linewidth]{fig/wikieptable} \caption{The Wikipedia table with hte 2014 EP election in the UK }\label{fig:wikieptable}
  \end{figure}
  In the figure, the title of the table is highlighted. If you copy the
  title and look up the source code of the page\footnote{To look up the
    source in Google Chrome, simply right click anywhere on the webpage
    and select \texttt{View\ Page\ Source}. If in doubt on how to find the
    source code, you can google the name of your browser and ``view souce
    code''.}, you can search for the table in the source code. This will
  show you what the code looks like for the table. To see the code for all
  our tables, we can simply call \texttt{data\_table}.
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{data_table}
  \end{Highlighting}
  \end{Shaded}
  We can see that this table is number 16 in our object. We save this
  table in the object \texttt{data\_results}.
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{data_results <-}\StringTok{ }\NormalTok{data_table[[}\DecValTok{16}\NormalTok{]]}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{library}\NormalTok{(}\StringTok{"httpuv"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{trump_tweets <-}\StringTok{ }\KeywordTok{get_timeline}\NormalTok{(}\StringTok{"realDonaldTrump"}\NormalTok{, }\DataTypeTok{n =} \DecValTok{100}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{news <-}\StringTok{ }\KeywordTok{search_tweets}\NormalTok{(}\StringTok{"filter:news"}\NormalTok{, }\DataTypeTok{n =} \DecValTok{100}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{brexit_news <-}\StringTok{ }\KeywordTok{search_tweets}\NormalTok{(}\StringTok{"brexit filter:news"}\NormalTok{, }\DataTypeTok{n =} \DecValTok{100}\NormalTok{, }\DataTypeTok{include_rts =} \OtherTok{FALSE}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{videos <-}\StringTok{ }\KeywordTok{search_tweets}\NormalTok{(}\StringTok{"filter:video"}\NormalTok{, }\DataTypeTok{n =} \DecValTok{100}\NormalTok{, }\DataTypeTok{include_rts =} \OtherTok{FALSE}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{lookup_tweets}\NormalTok{(}\StringTok{"1065623990746710022"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  \chapter*{(PART) Data visualisation}\label{part-data-visualisation}
  \addcontentsline{toc}{chapter}{(PART) Data visualisation}
  
  \chapter{\texorpdfstring{Introduction to
  \texttt{ggplot2}}{Introduction to ggplot2}}\label{dataviz}
  ...
  \end{Highlighting}
  \end{Shaded}
  \includegraphics{qpolr_files/figure-latex/unnamed-chunk-162-1.pdf}
  ...
  \section{Theme adjustments}\label{theme-adjustments}
  ...
  \chapter{Presenting distributions}\label{presenting-distributions}
  ...
  \chapter{Presenting relationships}\label{presenting-relationships}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-176-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-177-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-178-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-179-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-180-1} \end{center}
  
  \chapter{Manipulating plots}\label{manipulating-plots}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-181-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-182-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-183-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-184-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-185-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-186-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-187-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-188-1} \end{center}
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-189-1} \end{center}
  
  \section{Making multiple plots in
  ...
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-190-1} \end{center}
  
  However, now we have redundant information as we have a legend with no
  vital information not already visible in the figure. Below, we change
  the title of the figure, remove the legend and update the group names.
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# Recode variable to have more informative labels}
  \NormalTok{states}\OperatorTok{$}\NormalTok{abortlaw3_names <-}\StringTok{ }\KeywordTok{recode}\NormalTok{(states}\OperatorTok{$}\NormalTok{abortlaw3, }
                                   \StringTok{"0-5 restr"}\NormalTok{ =}\StringTok{ "Less than 6 restrictions"}\NormalTok{,}
                                   \StringTok{"6-8 restr"}\NormalTok{ =}\StringTok{ "Between 6 and 8 restrictions"}\NormalTok{,}
                                   \StringTok{"9-10 restr"}\NormalTok{ =}\StringTok{ "More than 8 restrictions"}
  \NormalTok{                                 )}
  ...
  \StringTok{  }\KeywordTok{scale_colour_manual}\NormalTok{(}\DataTypeTok{values =} \KeywordTok{c}\NormalTok{(}\StringTok{"red"}\NormalTok{, }\StringTok{"blue"}\NormalTok{, }\StringTok{"black"}\NormalTok{)) }\OperatorTok{+}
  \StringTok{  }\KeywordTok{labs}\NormalTok{(}
      \DataTypeTok{title =} \StringTok{"Abortion restrictions, abortions and the Obama vote"}\NormalTok{,}
      \DataTypeTok{subtitle =} \StringTok{"The relation between number of abortions and vote share for Obama"}\NormalTok{,}
      \DataTypeTok{caption =} \StringTok{"Data from the poliscidata R package"}\NormalTok{,}
      \DataTypeTok{colour =} \StringTok{"Abortion restrictions"}\NormalTok{,}
      \DataTypeTok{y =} \StringTok{"Obama vote share in 2012"}\NormalTok{,}
      \DataTypeTok{x =} \StringTok{"Number of abortions per 1,000 women aged 15-44 in 2008"}
  \NormalTok{  ) }\OperatorTok{+}
  \StringTok{  }\KeywordTok{facet_grid}\NormalTok{(}\OperatorTok{~}\StringTok{ }\NormalTok{abortlaw3_names) }\OperatorTok{+}
  \StringTok{  }\CommentTok{# Remove the legend}
  \StringTok{  }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{legend.position=}\StringTok{"none"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  \begin{center}\includegraphics{qpolr_files/figure-latex/unnamed-chunk-191-1} \end{center}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{glance}\NormalTok{(reg_obama)}
  \end{Highlighting}
  \end{Shaded}
  \begin{verbatim}
  # A tibble: 1 x 11
  ...
  *     <dbl>         <dbl> <dbl>     <dbl>   <dbl> <int>  <dbl> <dbl> <dbl>
  ...
  \end{Highlighting}
  \end{Shaded}
  \includegraphics{qpolr_files/figure-latex/unnamed-chunk-205-1.pdf}
  ...
  \end{Highlighting}
  \end{Shaded}
  \includegraphics{qpolr_files/figure-latex/unnamed-chunk-210-1.pdf}
  ...
  \end{Highlighting}
  \end{Shaded}
  \includegraphics{qpolr_files/figure-latex/unnamed-chunk-211-1.pdf}
  ...
  \end{Highlighting}
  \end{Shaded}
  \includegraphics{qpolr_files/figure-latex/unnamed-chunk-213-1.pdf}
  ...
  \end{Highlighting}
  \end{Shaded}
  \includegraphics{qpolr_files/figure-latex/unnamed-chunk-214-1.pdf}
  ...
  \end{Highlighting}
  \end{Shaded}
  \includegraphics{qpolr_files/figure-latex/unnamed-chunk-215-1.pdf}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{stargazer}\NormalTok{(reg_obama, }\DataTypeTok{type =} \StringTok{"text"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  \begin{verbatim}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{stargazer}\NormalTok{(reg_obama, reg_obama_full, }\DataTypeTok{type =} \StringTok{"text"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  \begin{verbatim}
  ...
  \begin{Highlighting}[]
  \KeywordTok{stargazer}\NormalTok{(reg_obama, reg_obama_full, }\DataTypeTok{type =} \StringTok{"text"}\NormalTok{,}
            \DataTypeTok{out=}\StringTok{"tab-regression.htm"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \hypertarget{refs}{}
  \hypertarget{ref-wdi}{}
  Arel-Bundock, V. (2018). \emph{WDI: World development indicators (world
  bank)}. Retrieved from \url{https://CRAN.R-project.org/package=WDI}
  
  \hypertarget{ref-chanetal2016}{}
  Chan, C., Chan, G. C. H., \& Leeper, T. J. (2016). \emph{Rio: A
  swiss-army knife for data file i/o}.
  
  \hypertarget{ref-essurvey}{}
  Cimentada, J. (2018). \emph{Essurvey: Download data from the european
  social survey on the fly}. Retrieved from
  \url{https://CRAN.R-project.org/package=essurvey}
  
  \hypertarget{ref-vdem}{}
  Coppedge, M., Gerring, J., Lindberg, S. I., Skaaning, S.-E., Teorell,
  J., Altman, D., \ldots{} Staton, J. (2017). V-Dem Codebook v7.1.
  Varieties of Democracy (V-Dem) Project. Retrieved from
  ...
  \hypertarget{ref-manifestoR}{}
  Lewandowski, J., \& Merz, N. (2018). \emph{ManifestoR: Access and
  process data and documents of the manifesto project}. Retrieved from
